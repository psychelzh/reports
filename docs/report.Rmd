...
users_participation <- users %>%
  left_join(school_info, by = "school") %>%
  mutate(is_finished = user_id %in% ability_scores$user_id) %>%
...
本次测评计划测评**`r nrow(users_participation)`**（初中：`r nrow(filter(users_participation, edu_stage == "初中"))`，小学：`r nrow(filter(users_participation, edu_stage == "小学"))`）名学生，实际完成测评学生数目为**`r nrow(filter(users_participation, is_finished))`**（初中：`r nrow(filter(users_participation, is_finished, edu_stage == "初中"))`，小学：`r nrow(filter(users_participation, is_finished, edu_stage == "小学"))`）名。
...
图\@ref(fig:school-summary)中展示了参与本次测评的**`r n_distinct(users_participation$school)`**所学校完成情况。其中未完成测评的人数是指没有任何测评数据的学生数目，这些学生将不包括在后面的报告中。
