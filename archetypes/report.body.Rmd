...
ab_type_stats %>%
  ...
  left_join(ab_type_prop, by = "ab_type") %>%
  mutate(
    grade = factor(grade, params$grade_order),
    ab_type = factor(
      ab_type, c("全面突出", "平衡发展", "有待改进", "亟待提升", "不均衡")
    )
  ) %>%
...
图\@ref(fig:plot-score-sex-diff-{{edu_stage_code}})展示了本次测评所有小学学生男女生的各能力表现差异。整体上看，`r descr_sexdiff`。
...
图\@ref(fig:plot-abtype-sex-diff-{{edu_stage_code}})展示了本次测评所有小学学生男女生的各能力类型分布情况。从图中可以看出，能力类型分布男女区别不大。
